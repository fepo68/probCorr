function Phi = designMatrix(X,basis,kern,varargin)
...
if strcmp(basis,'polynomial')
    ...
    if k==1
        ...
        Phi = [bias X];
        for i = 2:k
            Phi = horzcat(Phi, kern.variance*(kern.weightVariance*(X).^i));
            %Creating the Phi depending on the the value of k(=degree).
        end
